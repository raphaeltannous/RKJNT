...
RKJNT_newref()
{
    return calloc(1, sizeof(RKJNT_ref));
}
...
static bool
RKJNT_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
RKJNT_book_matches(const RKJNT_book *book, const char *s)
{
    return RKJNT_bookequal(book->name, s, false) ||
        RKJNT_bookequal(book->abbr, s, false) ||
        RKJNT_bookequal(book->name, s, true);
}

static int
RKJNT_book_fromname(const char *s)
{
    for (int i = 0; i < RKJNT_books_length; i++) {
        const RKJNT_book *book = &RKJNT_books[i];
        if (RKJNT_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
RKJNT_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
RKJNT_parseref(RKJNT_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (RKJNT_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = RKJNT_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
