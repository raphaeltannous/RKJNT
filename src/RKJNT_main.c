/*
RKJNT: Read the Word of God from your terminal
...
const char *
usage = "usage: RKJNT [flags] [reference...]\n"
    "\n"
    "Flags:\n"
...
main(int argc, char *argv[])
{
    ...
    for (int opt; (opt = getopt(argc, argv, "A:B:W:Cbepilh")) != -1; ) {
        ...
        case 'A':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "RKJNT: invalid flag value for -A\n\n%s", usage);
                return 1;
            }
            break;
        case 'B':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "RKJNT: invalid flag value for -B\n\n%s", usage);
                return 1;
            }
            break;
        case 'W':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "RKJNT: invalid flag value for -W\n\n%s", usage);
                return 1;
            }
        ...
        case 'h':
            ...
            return 0;
        case '?':
            fprintf(stderr, "RKJNT: invalid flag -%c\n\n%s", optopt, usage);
            return 1;
        }
    }
    ...
    if (argc == optind) {
        using_history();
        while (true) {
            char *input = readline("RKJNT> ");
            if (input == NULL) {
                break;
            }
            add_history(input);
            RKJNT_ref *ref = RKJNT_newref();
            int success = RKJNT_parseref(ref, input);
            free(input);
            if (success == 0) {
                RKJNT_render(ref, &config);
            }
            RKJNT_freeref(ref);
        }
    } else {
        ...
        RKJNT_ref *ref = RKJNT_newref();
        int success = RKJNT_parseref(ref, ref_str);
        free(ref_str);
        if (success == 0) {
            RKJNT_render(ref, &config);
        }
