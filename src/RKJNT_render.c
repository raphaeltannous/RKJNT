...
static void
RKJNT_output_verse(const RKJNT_verse *verse, FILE *f, const RKJNT_config *config)
{
    fprintf(
        f,
        config->highlighting ?
...
static bool
RKJNT_output(const RKJNT_ref *ref, FILE *f, const RKJNT_config *config)
{
    RKJNT_next_data next = {
        .current = 0,
        .next_match = -1,
        .matches = {
            {-1, -1},
    ...
    RKJNT_verse *last_printed = NULL;
    for (int verse_id; (verse_id = RKJNT_next_verse(ref, config, &next)) != -1; ) {
        RKJNT_verse *verse = &RKJNT_verses[verse_id];
        RKJNT_book *book = &RKJNT_books[verse->book - 1];
        ...
        if (config->pretty) {
            ...
            RKJNT_output_verse(verse, f, config);
        } else {
            fprintf(
                f,
                config->highlighting ?
...
static int
RKJNT_render_pretty(const RKJNT_ref *ref, const RKJNT_config *config)
{
    int fds[2];
    if (pipe(fds) == -1) {
        return 1;
    ...
    bool printed = RKJNT_output(ref, output, config);
    if (!printed) {
        kill(pid, SIGTERM);
    }
    fclose(output);
...
int
RKJNT_render(const RKJNT_ref *ref, const RKJNT_config *config)
{
    if (config->pretty) {
        return RKJNT_render_pretty(ref, config);
    }
    RKJNT_output(ref, stdout, config);
    return 0;
}
